...
function hand(size, strength, angle, r,g,b,a)
    ...
    local handcolor = resource.render_child("white")
    if strength == 2 then
       handcolor = resource.render_child("red")
    end
    handcolor:draw(0, -strength, size, strength)
    gl.popMatrix()
end
...
function node.render()
    gl.clear(0,0,0,1)
    if not bg then
        local white = resource.render_child("white")
        gl.pushMatrix()
        gl.translate(WIDTH/2, HEIGHT/2) 
        for i = 0, 59 do
            gl.pushMatrix()
            ...
            if i % 15 == 0 then
                white:draw(WIDTH/2.1-80, -10, WIDTH/2.1, 10, 1) 
            elseif i % 5 == 0 then
                white:draw(WIDTH/2.1-50, -10, WIDTH/2.1, 10, 1)
            else
                white:draw(WIDTH/2.1-5, -5, WIDTH/2.1, 5, 1)
            end
            gl.popMatrix()
        end
        gl.popMatrix()
